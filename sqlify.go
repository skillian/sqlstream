...
type exprWriter struct {
	...
	params     Parameters
	vs         expr.Values
	spareBytes *sliceCache[byte]
}

func (ew *exprWriter) init(ps Parameters) {
	if ew.aliases == nil {
		ew.aliases = make(map[expr.Var]string)
	}
	ew.params = ps
	ew.spareBytes = getSliceCache[byte]()
	ew.spareBytes.putSlice(make([]byte, 0, 32))
}
